...
| Setting | Value |
|---------|-------|
| Font sizes | Title: 14, Labels: 12, General: 11 |
...
$$\text{trainable \%} = \frac{\lvert\theta_{\text{train}}\rvert}{\lvert\theta_{\text{total}}\rvert} \times 100$$

### `estimate_memory(model, input_shape) → int`

Estimates GPU memory using:

$$\text{Memory} \approx \underbrace{4\lvert\theta\rvert}_{\text{weights}} + \underbrace{4\lvert\theta\rvert}_{\text{gradients}} + \underbrace{8\lvert\theta\rvert}_{\text{Adam states}} + \underbrace{\text{activations}}_{\text{data-dependent}}$$
...
Default: `n_samples=100`, `n_classes=5`, each image $32 \times 32 \times 3$.
