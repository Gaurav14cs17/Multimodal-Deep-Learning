...
|----------|---------------|
| `01_lora_from_scratch/01_lora_from_scratch.ipynb` | [![Open In Colab](https://colab.research.google.com/assets/colab-badge.svg)](https://colab.research.google.com/github/Gaurav14cs17/Multimodal-Deep-Learning/blob/main/04_Finetuning_LowCompute/01_lora_from_scratch/01_lora_from_scratch.ipynb) |
| `02_qlora_4bit_finetuning/02_qlora_4bit_finetuning.ipynb` | [![Open In Colab](https://colab.research.google.com/assets/colab-badge.svg)](https://colab.research.google.com/github/Gaurav14cs17/Multimodal-Deep-Learning/blob/main/04_Finetuning_LowCompute/02_qlora_4bit_finetuning/02_qlora_4bit_finetuning.ipynb) |
| `03_adapter_methods/03_adapter_methods.ipynb` | [![Open In Colab](https://colab.research.google.com/assets/colab-badge.svg)](https://colab.research.google.com/github/Gaurav14cs17/Multimodal-Deep-Learning/blob/main/04_Finetuning_LowCompute/03_adapter_methods/03_adapter_methods.ipynb) |
| `04_finetune_clip_custom_data/04_finetune_clip_custom_data.ipynb` | [![Open In Colab](https://colab.research.google.com/assets/colab-badge.svg)](https://colab.research.google.com/github/Gaurav14cs17/Multimodal-Deep-Learning/blob/main/04_Finetuning_LowCompute/04_finetune_clip_custom_data/04_finetune_clip_custom_data.ipynb) |
...
## Notebook 1: `01_lora_from_scratch/01_lora_from_scratch.ipynb` [![Open In Colab](https://colab.research.google.com/assets/colab-badge.svg)](https://colab.research.google.com/github/Gaurav14cs17/Multimodal-Deep-Learning/blob/main/04_Finetuning_LowCompute/01_lora_from_scratch/01_lora_from_scratch.ipynb)
...
## Notebook 2: `02_qlora_4bit_finetuning/02_qlora_4bit_finetuning.ipynb` [![Open In Colab](https://colab.research.google.com/assets/colab-badge.svg)](https://colab.research.google.com/github/Gaurav14cs17/Multimodal-Deep-Learning/blob/main/04_Finetuning_LowCompute/02_qlora_4bit_finetuning/02_qlora_4bit_finetuning.ipynb)
...
$$s = \frac{\lvert x \rvert_{\max}}{2^{b-1} - 1}, \qquad q = \text{round}\!\left(\frac{x}{s}\right), \qquad \hat{x} = s \cdot q$$
...
## Notebook 3: `03_adapter_methods/03_adapter_methods.ipynb` [![Open In Colab](https://colab.research.google.com/assets/colab-badge.svg)](https://colab.research.google.com/github/Gaurav14cs17/Multimodal-Deep-Learning/blob/main/04_Finetuning_LowCompute/03_adapter_methods/03_adapter_methods.ipynb)
...
## Notebook 4: `04_finetune_clip_custom_data/04_finetune_clip_custom_data.ipynb` [![Open In Colab](https://colab.research.google.com/assets/colab-badge.svg)](https://colab.research.google.com/github/Gaurav14cs17/Multimodal-Deep-Learning/blob/main/04_Finetuning_LowCompute/04_finetune_clip_custom_data/04_finetune_clip_custom_data.ipynb)
...
## PEFT Meets Reinforcement Learning — LoRA + DPO / GRPO

Modern alignment pipelines don't full-finetune during RL — they attach LoRA adapters and optimize only those. This keeps memory low and lets you merge or discard the alignment layer independently.

### Why Combine PEFT with RL?

```
  Memory comparison for 7B model alignment:

  Full-parameter DPO:        LoRA DPO (r=16):          QLoRA DPO (NF4, r=16):
  ┌──────────────────┐       ┌──────────────────┐      ┌──────────────────┐
  │ Policy    : 28 GB│       │ Base (frozen):14GB│      │ Base NF4  : 3.5GB│
  │ Reference : 28 GB│       │ LoRA A,B   : 50MB│      │ LoRA fp16 : 50MB │
  │ Optimizer : 56 GB│       │ Reference  : 14GB│      │ Reference : 3.5GB│
  │                  │       │ Optimizer  : 100MB│      │ Optimizer : 100MB│
  ├──────────────────┤       ├──────────────────┤      ├──────────────────┤
  │ TOTAL    ≈112 GB │       │ TOTAL     ≈ 28 GB│      │ TOTAL    ≈  7 GB │
  └──────────────────┘       └──────────────────┘      └──────────────────┘
        ✗ 2× A100                 ✓ 1× A100                ✓ 1× RTX 4090
```

### Mathematical Formulation

**DPO + LoRA** — only the LoRA parameters $\Delta\theta = BA$ are updated:

$$\mathcal{L}_{\text{DPO-LoRA}} = -\mathbb{E}\left[\log \sigma\!\left(\beta \log \frac{\pi_{W_0 + BA}(y_w \mid x)}{\pi_{W_0}(y_w \mid x)} - \beta \log \frac{\pi_{W_0 + BA}(y_l \mid x)}{\pi_{W_0}(y_l \mid x)}\right)\right]$$

The frozen base model $W_0$ serves as the reference policy for free — no separate reference copy needed:

$$\pi_{\text{ref}}(y \mid x) = \pi_{W_0}(y \mid x), \qquad \pi_\theta(y \mid x) = \pi_{W_0 + \frac{\alpha}{r}BA}(y \mid x)$$

**GRPO + QLoRA** — sample $G$ responses, score with verifiable reward, update adapters:

$$A_i = \frac{R(x, y_i) - \text{mean}(\lbrace R_j \rbrace_{j=1}^{G})}{\text{std}(\lbrace R_j \rbrace_{j=1}^{G})}, \qquad r_i = \frac{\pi_{W_{\text{NF4}} + BA}(y_i \mid x)}{\pi_{W_{\text{NF4}}}(y_i \mid x)}$$

$$\mathcal{L}_{\text{GRPO-QLoRA}} = -\frac{1}{G}\sum_{i=1}^{G} \min\!\left(r_i A_i, \;\text{clip}(r_i, 1-\epsilon, 1+\epsilon)\, A_i\right) + \beta\, \text{KL}(\pi_\theta \| \pi_{\text{ref}})$$

### LoRA + DPO Training Pipeline
...
```
  ...
  ├───────────────────────────────────────────────────────────────────────┤
  │                                                                       │
  │  Step 1: Load base model + attach LoRA                                │
  │  ┌─────────────────────────────────────────┐                         │
  │  │ base_model = AutoModelForCausalLM(...)  │                         │
  │  │ lora_config = LoraConfig(r=16, α=32,    │                         │
  │  │     target_modules=["q_proj","v_proj"])  │                         │
  │  │ model = get_peft_model(base, lora_conf) │                         │
  │  └─────────────────────┬───────────────────┘                         │
  │                        │                                              │
  │  Step 2: Reference policy = frozen base (free!)                      │
  │  ┌─────────────────────▼───────────────────┐                         │
  │  │ ref_model = None  # DPO trainer uses    │                         │
  │  │ the base weights inside the PEFT model  │                         │
  │  │ π_ref(y|x) = π_{W₀}(y|x) at zero cost │                         │
  │  └─────────────────────┬───────────────────┘                         │
  │                        │                                              │
  │  Step 3: Train on preference pairs                                    │
  │  ┌─────────────────────▼───────────────────┐                         │
  │  │ for (x, y_w, y_l) in dataset:           │                         │
  │  │   logπ_θ(y_w) = forward(W₀ + BA, x,y_w)│                         │
  │  │   logπ_ref(y_w) = forward(W₀, x, y_w)  │                         │
  │  │   ... same for y_l ...                   │                         │
  │  │   loss = -log σ(β(Δlog_w - Δlog_l))     │                         │
  │  │   loss.backward()  # only updates B, A  │                         │
  │  └─────────────────────┬───────────────────┘                         │
  │                        │                                              │
  │  Step 4: Merge or serve as adapter                                    │
  │  ┌─────────────────────▼───────────────────┐                         │
  │  │ Option A: model.merge_and_unload()      │                         │
  │  │   → θ_merged = W₀ + (α/r)·BA           │                         │
  │  │   → single model, zero overhead         │                         │
  │  │                                          │                         │
  │  │ Option B: save adapter separately       │                         │
  │  │   → adapter_size ≈ 50 MB                │                         │
  │  │   → swap adapters at serving time       │                         │
  │  └─────────────────────────────────────────┘                         │
  │                                                                       │
  └───────────────────────────────────────────────────────────────────────┘
```
...
| Setting | LoRA + DPO | QLoRA + GRPO |
|---------|-----------|-------------|
| **Base model** | 7B (fp16/bf16) | 7B (NF4) |
| **LoRA rank** | $r = 16$ | $r = 16$ |
| **LoRA $\alpha$** | 32 | 32 |
| **Target modules** | q_proj, v_proj, k_proj, o_proj | q_proj, v_proj |
| **$\beta$ (KL penalty)** | 0.1 | 0.04 |
| **Learning rate** | $5 \times 10^{-7}$ | $1 \times 10^{-5}$ |
| **Batch size** | 4 (accum 4) | 4 (accum 4) |
| **Group size $G$** | — | 8 |
| **$\epsilon$ (clip)** | — | 0.2 |
| **Memory** | ~28 GB | ~7 GB |

### Key Insight: The Adapter Algebra of Alignment

After training, the alignment adapter can be composed with other task adapters using vector arithmetic:

$$\theta_{\text{aligned+task}} = W_0 + \frac{\alpha}{r}B_{\text{DPO}} A_{\text{DPO}} + \frac{\alpha'}{r'}B_{\text{task}} A_{\text{task}}$$

This is the **LoRA stacking** paradigm:

```
  Base Model (W₀, frozen)
       │
       ├── + LoRA_DPO (safety alignment, 50 MB)
       │
       ├── + LoRA_task_A (medical QA, 50 MB)
       │
       ├── + LoRA_task_B (code generation, 50 MB)
       │
       └── At inference: W₀ + LoRA_DPO + LoRA_task_A  (stack any combination)
```

$$h = W_0 x + \sum_{k=1}^{K} \frac{\alpha_k}{r_k} B_k A_k x$$

Total trainable parameters per adapter ($r=16$, $d=4096$, 32 layers, 4 modules per layer):

$$\lvert\theta_{\text{adapter}}\rvert = 2 \times r \times d \times L \times M = 2 \times 16 \times 4096 \times 32 \times 4 = 16{,}777{,}216 \approx 17\text{M}$$

That's **0.24%** of a 7B model — yet sufficient for full safety alignment via DPO.
...
| RL integration methods | 2 (LoRA+DPO, QLoRA+GRPO) |
| Key equation | $h = W_0 x + \frac{\alpha}{r} BAx$ |
| 7B full FT memory | 112 GB |
| 7B LoRA+DPO memory | ~28 GB |
| 7B QLoRA+GRPO memory | ~7 GB |
| Parameter savings (LoRA, $r=8$, $d=768$) | 97.9% |
| Alignment adapter size (7B, $r=16$) | ~50 MB (0.24% of model) |
...
**[05_Advanced_Topics/01_llava_architecture/01_llava_architecture.ipynb](../05_Advanced_Topics/01_llava_architecture/01_llava_architecture.ipynb)** [![Open In Colab](https://colab.research.google.com/assets/colab-badge.svg)](https://colab.research.google.com/github/Gaurav14cs17/Multimodal-Deep-Learning/blob/main/05_Advanced_Topics/01_llava_architecture/01_llava_architecture.ipynb) — how LLaVA connects vision to LLMs.
