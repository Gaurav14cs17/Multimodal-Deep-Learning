...
## Notebook 1: `01_llava_architecture/01_llava_architecture.ipynb` [![Open In Colab](https://colab.research.google.com/assets/colab-badge.svg)](https://colab.research.google.com/github/Gaurav14cs17/Multimodal-Deep-Learning/blob/main/05_Advanced_Topics/01_llava_architecture/01_llava_architecture.ipynb)
...
$$\lvert\theta_{\text{proj}}\rvert = \underbrace{D_v D_h}_{W_1} + \underbrace{D_h}_{b_1} + \underbrace{D_h D_t}_{W_2} + \underbrace{D_t}_{b_2}$$
...
$$\min_{\theta_{\text{proj}}} \mathcal{L}_{\text{LM}} = -\sum_{t=1}^{T'} \log P\!\left(y_t \mid y_1, \ldots, y_{t-1}, I;\; \theta_{\text{ViT}}^*,\; \theta_{\text{proj}},\; \theta_{\text{LLM}}^*\right)$$
...
$$\min_{\theta_{\text{proj}},\, A,\, B} \mathcal{L}_{\text{LM}} = -\sum_{t} \log P\!\left(y_t \mid y_1, \ldots, y_{t-1}, I;\; \theta_{\text{ViT}}^*,\; \theta_{\text{proj}},\; \theta_{\text{LLM}}^* + BA\right)$$
...
## Notebook 2: `02_multimodal_beyond_vision/02_multimodal_beyond_vision.ipynb` [![Open In Colab](https://colab.research.google.com/assets/colab-badge.svg)](https://colab.research.google.com/github/Gaurav14cs17/Multimodal-Deep-Learning/blob/main/05_Advanced_Topics/02_multimodal_beyond_vision/02_multimodal_beyond_vision.ipynb)
...
$$\text{MelSpec}(f, t) = \sum_{k} \lvert X(k, t) \rvert^2 \, H_f(k)$$
...
## Notebook 3: `03_efficient_deployment/03_efficient_deployment.ipynb` [![Open In Colab](https://colab.research.google.com/assets/colab-badge.svg)](https://colab.research.google.com/github/Gaurav14cs17/Multimodal-Deep-Learning/blob/main/05_Advanced_Topics/03_efficient_deployment/03_efficient_deployment.ipynb)
...
## Weight-Space Merging — Combining Specialists Without Retraining

After fine-tuning multiple models (SFT, DPO, domain-specific), you can combine them in weight space — no additional training required. This is the **Stage 3** of modern pipelines (Module 03 introduces the theory; here we cover deployment-focused aspects).

### The Core Idea

Every fine-tuned model defines a **task vector** — the delta from the base model:

$$\tau_k = \theta_k^{\text{fine}} - \theta^{\text{base}}$$

Merging combines multiple task vectors into one model:

```
  Base Model θ_base
       │
       ├── Fine-tune on Task A → θ_A    →  τ_A = θ_A - θ_base
       ├── Fine-tune on Task B → θ_B    →  τ_B = θ_B - θ_base
       ├── DPO alignment       → θ_DPO  →  τ_DPO = θ_DPO - θ_base
       │
       └── Merge:  θ_merged = θ_base + λ₁τ_A + λ₂τ_B + λ₃τ_DPO
                                            ↓
                              One model with all capabilities
```

### Three Merging Methods for Deployment

**1. Linear Interpolation (Model Souping)**

$$\theta_{\text{merged}} = \lambda\, \theta_A + (1 - \lambda)\, \theta_B, \qquad \lambda \in [0, 1]$$

Best when $\theta_A$ and $\theta_B$ are fine-tuned from the same base.

**2. Task Arithmetic**

$$\theta_{\text{merged}} = \theta^{\text{base}} + \sum_{k=1}^{K} \lambda_k \tau_k$$

More flexible — different scaling per task. Can even **negate** a task vector ($\lambda_k < 0$) to *remove* a capability (e.g., toxicity):

$$\theta_{\text{detoxified}} = \theta^{\text{base}} - 0.5 \cdot \tau_{\text{toxic}}$$

**3. TIES Merging** (Trim, Elect Sign, Disjoint Merge)

Addresses interference between task vectors:

$$
\text{TIES}(\lbrace\tau_k\rbrace) = \begin{cases} \text{Trim: } \tilde{\tau}_k^{(j)} = 0 & \text{if } \lvert\tau_k^{(j)}\rvert < p_{\text{thresh}} \\[6pt] \text{Elect: } s^{(j)} = \text{sign}\!\left(\sum_k \tilde{\tau}_k^{(j)}\right) & \text{(majority vote per param)} \\[6pt] \text{Merge: } \tau_{\text{merged}}^{(j)} = \frac{1}{\lvert\mathcal{A}_j\rvert}\sum_{k \in \mathcal{A}_j} \tilde{\tau}_k^{(j)} & \text{where } \text{sign}(\tilde{\tau}_k^{(j)}) = s^{(j)} \end{cases}
$$

```
  TIES Merging Pipeline:

  τ_A: [0.01, -0.8, 0.3, 0.02, -0.6]     τ_B: [-0.03, 0.7, 0.4, -0.01, 0.5]
           │                                          │
           ▼  Trim (remove < 0.05)                    ▼  Trim
       [0,    -0.8, 0.3,  0,   -0.6]           [0,    0.7, 0.4,  0,    0.5]
                    │                                       │
                    └──────────────┬────────────────────────┘
                                   ▼  Elect Sign (majority vote)
                           signs: [0,   -1,  +1,   0,   -1]
                                   │
                                   ▼  Disjoint Merge (avg agreeing signs)
                           merged: [0, -0.8, 0.35, 0, -0.6]
```

### Merging LoRA Adapters (Lightweight Merging)

You can merge at the adapter level before committing to full-model merging:

$$W_{\text{merged}} = W_0 + \sum_{k=1}^{K} \frac{\alpha_k}{r_k} \lambda_k B_k A_k$$

This is dramatically cheaper — merge $K$ adapters of ~50 MB each instead of $K$ full 14 GB models:

| Method | Storage per Specialist | Merge Cost | Quality |
|--------|----------------------|------------|---------|
| Full model merging | 14 GB × $K$ | Seconds (just add) | High |
| LoRA adapter merging | 50 MB × $K$ | Seconds | High |
| TIES + LoRA | 50 MB × $K$ | Seconds | Highest (less interference) |
| Retraining from scratch | $K$ × training budget | Hours–days | Baseline |

### Deploying Merged Models

```
  Deployment pipeline for merged models:

  ┌──────────────────────────────────────────────────────────────────┐
  │                                                                  │
  │  LoRA_DPO (50MB) + LoRA_medical (50MB) + LoRA_code (50MB)       │
  │        │                     │                    │              │
  │        └─────────────────────┼────────────────────┘              │
  │                              ▼                                   │
  │                    Merge (task arithmetic)                        │
  │                    W = W₀ + λ₁B₁A₁ + λ₂B₂A₂ + λ₃B₃A₃          │
  │                              │                                   │
  │                              ▼                                   │
  │                    merge_and_unload()                             │
  ...
  │                    ┌─────────┼──────────┐                        │
  │                    ▼         ▼          ▼                        │
  │               torch.compile  ONNX      vLLM                      │
  │               (quick)        (edge)    (scale)                   │
  │                                                                  │
  └──────────────────────────────────────────────────────────────────┘
```

### Serving RL-Aligned Multimodal Models at Scale

Modern VLMs (LLaVA + DPO/GRPO alignment) have unique serving challenges:

```
  Request: "Describe this medical image in detail"
                    │
                    ▼
  ┌───────────────────────────────────────┐
  │           vLLM + PagedAttention        │
  ...
  │     → 576 visual tokens               │
  │     → KV cache: 2×L×576×d per layer   │
  │                                        │
  │  2. Projector (2-layer MLP, 21M)      │
  │     → Map vision to LLM space         │
  │                                        │
  │  3. LLM (merged: base + DPO + task)   │
  │     → Causal generation with           │
  │       visual prefix                    │
  │                                        │
  │  Memory budget for 7B VLM:            │
  │  ┌──────────────────────────────┐     │
  │  │ Model weights (FP16) : 14 GB │     │
  │  │ KV cache (2K ctx)    :  2 GB │     │
  │  │ Visual tokens/batch  :  1 GB │     │
  │  │ Overhead              :  1 GB │     │
  │  │ Total                : 18 GB │     │
  │  └──────────────────────────────┘     │
  │                                        │
  ...
  │  • PagedAttention (no KV waste)        │
  │  • FP8 KV cache (halve KV memory)     │
  │  • Prefix caching (shared system       │
  │    prompt across requests)             │
  └───────────────────────────────────────┘
```

**KV Cache Memory** for visual + text tokens:

$$M_{\text{KV}} = 2 \times L \times (T_{\text{vis}} + T_{\text{text}}) \times d \times \text{bytes}$$

For a 7B model ($L=32$, $d=4096$) with 576 visual + 512 text tokens, in FP16:

$$M_{\text{KV}} = 2 \times 32 \times (576 + 512) \times 4096 \times 2 = 570 \text{ MB per request}$$

With **FP8 KV cache**: $285$ MB per request — doubling batch capacity.
...
| Merging methods | 3 (Linear Interpolation, Task Arithmetic, TIES) |
| Projector parameters (LLaVA) | ~21M |
| Training data (LLaVA) | 595K pretrain + 150K instruct |
| KV cache per request (7B VLM, FP16) | ~570 MB |
| KV cache per request (7B VLM, FP8) | ~285 MB |
| Runs on CPU | Yes |
